...
var mui = (function(document, undefined) {
	...
	/**
	 * 将 fn 缓存一段时间后, 再被调用执行 此方法为了避免在 ms 段时间内, 执行 fn 多次. 常用于 resize , scroll ,
	 * mousemove 等连续性事件中; 当 ms 设置为 -1, 表示立即执行 fn, 即和直接调用 fn 一样; 调用返回函数的 stop
	 * 停止最后一次的 buffer 效果
	 * 
	 * @param {Object}
	 *            fn
	...
	var delegateFn = function(element, event, selector, callback) {
		return function(e) {
			...
			for (; target && target !== document; target = target.parentNode) {
				...
				var matches = {};
				$.each(callbackObjs, function(selector, callbacks) { // same
																		// selector
...
/*
 * A shim for non ES5 supporting browsers. Adds function bind to Function
 * prototype, so that you can do partial application. Works even with the nasty
 * thing, where the first word is the opposite of extranet, the second one is
 * the profession of Columbus, and the version number is 9, flipped 180 degrees.
 */
...
Function.prototype.bind = Function.prototype.bind || function(to) {
	...
	var partial = Array.prototype.splice.call(arguments, 1),
		...
		fn = this;
	var bound = function() {
			// Join the already applied arguments to the now called ones (after
			// converting to an array again).
			var args = partial.concat(Array.prototype.splice.call(arguments, 0));
			// If not being called as a constructor
			if (!(this instanceof bound)) {
				// return the result of the function called bound to target and
				// partially applied.
				return fn.apply(to, args);
			}
			// If being called as a constructor, apply the function bound to
			// self.
			fn.apply(this, args);
		}
		...
		switch (event.type) {
			case $.EVENT_END:
				...
				if (touch.distance < options.tapMaxDistance && touch.deltaTime < options.tapMaxTime) {
					if ($.options.gestureConfig.doubletap && lastTarget && (lastTarget === target)) { // same
																										// target
...
(function($) {
	 ...
	 * @param {object}
	 *            options
	 *            可选:参数,等待,窗口,显示配置{params:{},waiting:{},styles:{},show:{}}
	 */
			...
			if (parent) {
				...
				wobj.canBack(function(e) {
					...
					if (e.canBack) { // webview history back
						...
						// fixed by fxy
						// 此处不应该用opener判断，因为用户有可能自己close掉当前窗口的opener。这样的话。opener就为空了，导致不能执行close
	...
	$.plusReady(function() {
		$.ajaxSettings = $.extend($.ajaxSettings, {
			xhr: function(settings) {
				...
				if ($.os.ios && window.webkit && window.webkit.messageHandlers) { // wkwebview下同样使用5+
																					// xhr
                    return new plus.net.XMLHttpRequest();
                }
				return new window.XMLHttpRequest();
...
(function($, window, document, undefined) {
	...
	var Scroll = $.Class.extend({
		...
		scrollTo: function(x, y, time, easing) {
			...
			// this.isInTransition = time > 0 && (this.lastX != x || this.lastY
			// != y);
...
(function($, window, document, undefined) {
	...
	$.fn.pullRefresh = function(options) {
		if (this.length === 1) {
			...
			// var methodValue = pullRefreshApi[options].apply(pullRefreshApi,
...
(function($, window, document, name) {
	...
	var OffCanvas = $.Class.extend({
		...
		refresh: function(offCanvas) {
			// offCanvas && !offCanvas.classList.contains(CLASS_ACTIVE) &&
	...
	window.addEventListener('tap', function(e) {
		...
		for (; target && target !== document; target = target.parentNode) {
			if (target.tagName === 'A' && target.hash && target.hash === ('#' + $.targets.offcanvas.id)) {
				e.detail && e.detail.gesture && e.detail.gesture.preventDefault(); // fixed
																					// hashchange
...
(function($, name) {
	...
	var handle = function(event, target) {
		...
		if (typeof className !== 'string') { // svg
...
(function($, window, document, name) {
	...
	var togglePopover = function(popover, anchor, state) {
		...
		if (popover.classList.contains(CLASS_BAR_POPOVER) || popover.classList.contains(CLASS_ACTION_POPOVER)) { // navBar
			if (popover.classList.contains(CLASS_ACTION_POPOVER)) { // action
																	// sheet
																	// popover
				isActionSheet = true;
				...
				// popover.style.left = (Math.min(Math.max(offsetLeft,
				// defaultPadding), innerWidth - offsetWidth - defaultPadding))
				// + "px";
				// } else {
...
(function($, window, document, name, undefined) {
	...
	window.addEventListener('tap', function(e) {
		...
		if (!targetBody.classList.contains(CLASS_CONTROL_CONTENT)) { // tab
																		// bar
																		// popover
	...
	window.addEventListener($.EVENT_START, function(event) {
		...
		for (; target && target !== document; target = target.parentNode) {
			if (target.classList) {
				...
				if (classList.contains(CLASS_COLLAPSE_CONTENT)) { // collapse
																	// content
					break;
				}
...
(function($, document) {
	 ...
	 * 
	 * @param {Object}
	 *            container 可选，默认body，支持selector,DOM Node,mui wrapper
	 * @param {Object}
	 *            progress 可选，undefined表示循环，数字表示具体进度
	 * @param {Object}
	 *            color 可选，指定颜色样式(目前暂未提供实际样式，可暂时不暴露此参数)
	 */
...
/**
 * 数字输入框 varstion 1.0.1 by Houfeng Houfeng@DCloud.io
 */
